<script>
    ...
    export let currentSelectedColletion = "users"
</script>

<div class="border-r min-w-56 lg:min-w-64 h-full flex flex-col items-center">
    <h1 class="mt-4 text-lg font-semibold">LeonDB</h1>
    <div class="w-full px-6 mt-4 border-t py-1">
        <h2 class="text-center text-neutral-400 mt-2">Collections</h2>
    </div>
    <div class="flex flex-col mt-4 w-full px-6 gap-y-6">
        {#each $collections as collection}
            <button title={collection.name} on:click={() => currentSelectedColletion = collection.name} class="rounded w-full text-center py-2 flex items-center px-2 {currentSelectedColletion.toLowerCase() === collection.name.toLowerCase() ? "bg-neutral-200 text-black" : "text-neutral-600"} hover:bg-neutral-200 transition">
